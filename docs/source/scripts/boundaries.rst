...
Rhino 支持能力与浏览器不同。为了兼容性，建议使用保守语法。
...
推荐使用 Java 反射 Array::
   ...
   const n = ArrayReflect.getLength(files);

   for (let i = 0; i < n; i++) {
     const item = ArrayReflect.get(files, i);
   }
...
如果已经 dump 出 dex，并且想检查某个方法是否包含目标特征，优先使用 Java 层接口，不要在 JS 里手动遍历 dex 文件。
...
示例：检查 ``pc.a.d()Z``::
   ...
   xposed.i("Inspect", "found=" + boolGet(inspected, "found"));
   xposed.i("Inspect", "featuresOk=" + boolGet(inspected, "featuresOk"));
   xposed.i("Inspect", "smali.head=" + strGet(inspected, "smaliHead"));
...
- Java Map 用 ``map.get("key")``。
- Java 数组用 ``java.lang.reflect.Array``。
