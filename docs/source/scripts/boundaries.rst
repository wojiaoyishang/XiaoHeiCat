...
可参考 Rhino 官方语法支持页面：`Rhino engine compatibility table <https://mozilla.github.io/rhino/compat/engines.html>`_。
该页面用于确认 Rhino 与常见 JS 引擎的语法兼容性差异；XiaoHeiHook 文档中的推荐写法仍以当前 Android / LSPosed 环境实测结果为准。
...
``java.full``
   允许使用 ``Java.use(...)`` 访问 Java 类、反射、文件、集合等。
...
``Java.use(className)`` 返回 ``JavaClassWrapper``，不是裸 ``java.lang.Class``。
...
示例::
   ...
   function installLoadClassWatcher(context, appLoader) {
     ...
     const loadClass = ClassLoader.getDeclaredMethod("loadClass", "java.lang.String");
